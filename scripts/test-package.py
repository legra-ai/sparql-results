...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=f"{package['name']}-package-") as directory:
        ...
        with tarfile.open(target / "package" / f"{stem}.crate") as archive:
            ...
            for required in ("LICENSE-MIT", "LICENSE-APACHE", "README.md"):
                if f"{stem}/{required}" not in members:
                    raise RuntimeError(f"Published archive omits {required}")
        ...
        for selection in ["--all-targets", "--doc"]:
            subprocess.run(
                ["cargo", "test", "--locked", "--all-features", selection,
